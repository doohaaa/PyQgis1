...
sido_nm = ['\'강원도\'', '\'경기도\'', '\'경상남도\'', '\'경상북도\'', '\'광주광역시\'', '\'대구광역시\'', '\'대전광역시\'', '\'부산광역시\'',
           '\'서울특별시\'', '\'세종특별자치시\'',
...
for i in list:
    ...
    for j in sido_nm:
        ...
        layer.selectByExpression("\"Type\"= 'Urban Center' and \"sido_nm\"= %s" %j, QgsVectorLayer.SetSelection)
        features_count = layer.selectedFeatureCount()
        print("UrbanCenter : %d" % features_count)

        layer.selectByExpression("\"Type\"= 'Urban Cluster' and \"sido_nm\"=  %s" %j, QgsVectorLayer.SetSelection)
        features_count = layer.selectedFeatureCount()
        print("UrbanCluster : %d" % features_count)

        layer.selectByExpression("\"Type\"= 'Rural' and \"sido_nm\"=  %s" %j, QgsVectorLayer.SetSelection)
        features_count = layer.selectedFeatureCount()
